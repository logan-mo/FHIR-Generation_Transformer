...
tokenized_seed = tokenizer(list(df['seed'].iloc[:100]), max_length=max_seed_length, padding='max_length', truncation=True, return_tensors="pt")
tokenized_fhir = tokenizer(list(df['FHIR'].iloc[:100]), max_length=max_fhir_length, padding='max_length', truncation=True, return_tensors="pt")
...
transformer = Transformer(src_vocab_size, tgt_vocab_size, d_model, num_heads, num_layers, d_ff, max(max_fhir_length, max_seed_length), dropout, device).to(device)

# Generate random sample data
src_data = tokenized_seed['input_ids'].to(device)
tgt_data = tokenized_fhir['input_ids'].to(device)

print(f"Input Shape: {src_data.shape}", f"Output Shape: {tgt_data.shape}")
